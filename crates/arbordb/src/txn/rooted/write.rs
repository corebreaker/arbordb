...
use super::super::{IndexQuery, WriteTxn};
use crate::{
...
impl<'a> RootedWrite<'a> {
    ...
    /// Removes the a-node at `path` (relative to the root). Reports if it existed.
    ...
    /// Loads a typed value from the file at `path` (relative to the root), reading
    /// this transaction's own uncommitted state.
    ...
    /// Opens a read accessor over the file at `path` (relative to the root). For a
    /// *mutable* accessor use [`fetch_mut`](Self::fetch_mut).
...
impl<P: IntoArborPath> JsonExporter<P> for RootedWrite<'_> {
...
impl<P: IntoArborPath> YamlExporter<P> for RootedWrite<'_> {
