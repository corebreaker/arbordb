//! [`NodeTimestamps`] — the public, decoded form of an a-node's created / modified /
...
/// The created / modified / accessed timestamps of an a-node (a file or directory),
/// available with the `entry-timestamps` feature.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct NodeTimestamps {
    /// When the a-node was first created.
    created: DateTime<Utc>,

    /// When the a-node's content was last modified.
    modified: DateTime<Utc>,

    /// When the a-node was last read. Updated lazily: reads buffer the access time in
...
impl NodeTimestamps {
    ...
    /// When the a-node was last read (see the field docs for the lazy-flush caveat).
