...
**Flows.** *Read:* APath → (walk dirs, cache-amortized) `AKey` → borrowed file blob → `ArchivedValue` → an `ArborXxx` accessor whose `get()` methods walk the bytes zero-copy → `Scalar`/typed field. *Write:* `store(apath, &T)` drives `AData::store` into an in-memory `Value` builder, encodes it to one blob, writes the file node and links it into its parent directory. A partial write that overwrites a scalar with one of the same byte width patches the blob's bytes in place (no decode, no re-encode); any other partial write re-encodes and rewrites the file blob. The large zero-copy win remains on reads.
...
- **The zero-copy win is on reads; a same-width scalar overwrite skips re-encoding.** `ArchivedValue` borrows the redb page. Overwriting a scalar with one of the same byte width patches those bytes in place — no decode, no re-encode — though redb still rewrites the value entry whole (it writes values whole and stays opaque, so persistence is never sub-`O(blob)`). Any other write re-encodes one blob.
